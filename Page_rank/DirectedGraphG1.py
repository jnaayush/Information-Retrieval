...
source_path = "HTMLPages/"
destination_path = "DocIDs/"


filename = "links_Carbon_footprint.csv"
...
def constructGraph():
    graph = "G1.txt"
    fgraph = open(graph,"w")
    fileList = os.listdir("DocIDs/")
    filename = "DocID_Carbon_footPrint.csv"
    with open(filename,"r") as file:
        for line in file:
            line = line.replace("\n","")
            fgraph.write(line)
            ...
            for docID_file in fileList:
                docId_incoming = open("DocIDs/" + docID_file,"r")
